...
import { 
  Search, Filter, Calendar, Download, Edit2, Trash2, 
  Utensils, Car, Lightbulb, Film, ShoppingBag, X, ChevronDown 
} from 'lucide-react';
...
function Expense({ darkMode, setDarkMode }) {
  ...
  const [allExpenses, setAllExpenses] = useState(() => {
    ...
    if (saved) {
      ...
      return Object.values(parsed).flat().sort((a, b) => new Date(b.date) - new Date(a.date));
    }
    return [];
  });
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-black' : 'bg-white'}`}>
      ...
      <main className="p-3 sm:p-4 lg:p-6 max-w-7xl mx-auto">
        ...
        <div className={`${isMobile && !showMobileFilters ? 'hidden' : 'block'} mb-4`}>
          <div className={`p-4 rounded-xl border ${darkMode ? 'bg-black border-gray-800' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center justify-between mb-4">
              ...
              {hasActiveFilters && (
                <button onClick={clearFilters} className="text-sm text-blue-500 flex items-center gap-1">
        ...
        {/* Summary Bar - Mobile */}
        {isMobile && (
          <div className={`flex items-center justify-between p-3 rounded-xl mb-4 ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
          ...
          {!isMobile && (
            <div className="flex justify-between items-center mb-4">
              ...
              <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Total: <span className="font-bold">${totalAmount.toFixed(2)}</span> • {filteredExpenses.length} items
              </span>
            </div>
          )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                      ...
                      <div className="hidden sm:flex gap-1 opacity-0 group-hover:opacity-100">
                        <button className={`p-1.5 rounded-lg ${darkMode ? 'hover:bg-gray-800 text-gray-400' : 'hover:bg-gray-200 text-gray-500'}`}>
